...
function generate(row, col){
    ...
    for (i = 0; i < row; i++) {
        act += "<tr>";
        for (j = 0; j < col; j++) {
            act += "<td class='section' onclick='zi()'></td>";
            dex.push([min, max]);
    ...
    gri = Array.prototype.slice.call(gri);
}
...
function zi (){
    ...
    if(pos.classList.contains('aoe')){
        if(pos.children.length>0){
			pos.firstChild.classList.add('ashake');
			setTimeout(function() {
			pos.firstChild.classList.remove('ashake');
			}, 1000);
        }
        else {
			pos.classList.add('animated');
			pos.classList.add('shake');
			setTimeout(function() {
				f('animated');f('shake');
			}, 1000);
        }
        ...
        if (pos.firstElementChild.classList.contains('rouge')){
            if($('#tour').val() == 'rouge'){
                pion = pos.children[0];
                f('active'); f('range'); f('aoe');f('non');
                pos.classList += ' active';
            }
        }
            else if(pos.firstElementChild.classList.contains('bleu')){
                if($('#tour').val() == 'bleu'){
                    pion = pos.children[0];
                    f('active'); f('range'); f('aoe');f('non');
                    pos.classList += ' active';
    ...
    else if (pos.classList.contains('range')) {
        pos.appendChild(pion);
        f('active'); f('range');
    }
    if($('#tour').val() == 'bleu' ){
...
function cross(){
    ...
    row = Number(getID("row").value);
    col = Number(getID("col").value);
    dex = JSON.parse(localStorage.getItem("index"));
    let tot = dex.length-1, min = dex[pos][0], max = dex[pos][1];
    for (i = 0; i< row; i++){
        aInfB(pos+i, max, 'aoe', 0);
        aSupB(pos-i, min, 'aoe', 0);
    }
    for (i = 0; i< col; i++){
       aInfB(pos+col*i, tot, 'aoe', 0);
        aSupB(pos-col*i,   0, 'aoe', 0);
    }
}

function h(rrr, a, b, non){//portée/évite/classe/non
    pos = getPos();
    
	addClass(pos, 'active');
    f("range");f("aoe");f('noir')
    
    for (j=0; j<rrr;j++){
        if (j == 0){selection('active', b, a);}
        else {selection(b, b, a);}
    }
    if (non > 0){
        for (j=0; j<=non;j++){
            if (j == 0){selection('active', b, a, non);}
            else {selection('non', b, a, non);}
        }
    } 
}

function selection(a, c, b, non){//classe/évite/portée/non
    sel = new Array();
    
    for (i=0; i<gri.length; i++){
        if (gri[i].classList.contains(a)){ sel.push(i); }
    }
    
    if (non > 0){ for (i=0; i<sel.length; i++){ range(sel[i], c, b, 1); } }
    else{ for (i=0; i<sel.length; i++){ range(sel[i], c, b, 0); } }
}

function range(pos, a, b, d){//position/classe/evite/non
    if(typeof(gri[pos]) != 'undefined'){
        ...
        if (d){
            if(gri[pos].classList.contains('non')||gri[pos].classList.contains('active')){
                aSupB2(pos - 1  , min, a);
                aInfB2(pos + 1  , max, a);
                aSupB2(pos - col,   0, a);
                aInfB2(pos + col, tot, a);
            }
        }
        else{
...
function ifChild(a){ if (gri[a].children.length>0) return 1; else return 0; }

function addClass(a,b){ gri[a].classList.add(b); }
function supClass(a,b){ gri[a].classList.remove(b); }
...
function aSupB2(a,b,c){ if (a >= b) {supClass(a,c); addClass(a, 'non')} }
function aInfB2(a,b,c){ if (a <= b) {supClass(a,c); addClass(a, 'non')} }  
